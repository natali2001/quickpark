...
def book_parking_spot(request, parking_spot_id):
    """
    Book a parking spot and create a booking instance
    """
    if request.method == 'POST':
        ...
        if parking_spot.is_available():
            ...
            # update parking spot availability status
            parking_spot.is_booked = True
            parking_spot.save()

            # return success response
            return JsonResponse({'message': 'Parking spot booked successfully.'})
        else:
            # return error response if parking spot is not available
